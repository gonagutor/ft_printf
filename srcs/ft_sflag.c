...
/*   Updated: 2021/05/17 12:41:56 by gaguado-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_spadding(t_flags flg, char *str)
{
	...
	{
		if (len < flg.prec_mod)
			ret += ft_pcrepeatedly(' ', flg.flagqtt_mod - len);
		else
			ret += ft_pcrepeatedly(' ', flg.flagqtt_mod - flg.prec_mod);
		ret += ft_putstr_upton(str, flg.prec_mod);
	}
	else
	{
		if (flg.zero_mod)
			ret += ft_pcrepeatedly('0', flg.flagqtt_mod - len);
		else
			ret += ft_pcrepeatedly(' ', flg.flagqtt_mod - len);
		ret += ft_putstr_upton(str, len);
	}
	ret += ft_pcrepeatedly(' ', (-1 * flg.flagqtt_mod) - ret);
	return (ret);
}
